...
# for each sample, tally label count
# label_tally = np.zeros((num_samples, num_labels))
...
# Example predicted labels for N samples
    ...
    # print(rational_indices_list)
    # print(rational_indices_list[0])

    for approx_model in approx_model_names:
        data_dir = os.path.join('../data', 'movies', 'eval_x_approx', approx_model, 'top_' + str(num_tokens) + '_' + 'tokens')
        ...
        for i, rat_index_per_sample in enumerate(test_rationale_indices):
            # print(i, rational_indices_list[i])
            # print(rat_index_per_sample)
            rational_indices_list[i].extend(rat_index_per_sample)
            # print(rational_indices_list[i])
            # print()
    exp_per_token[num_tokens] = rational_indices_list
        # aggregate_explanations[num_tokens] = test_rationale_indices
...
#         pos_prob = np.exp(eval_logits[0,1])/np.sum(np.exp(eval_logits))
#         eval_pred_prob.append(pos_prob)
#         eval_pred_label.extend(np.argmax(eval_logits, axis = 1))
#     true_label_id = [model.config.label2id[l] for l in original_dataset['test']['label']]
#     eauroc = roc_auc_score(true_label_id, eval_pred_prob)
#     print('eAUROC', eauroc)
#     print('eACC', np.mean(np.array([model.config.id2label[l] for l in eval_pred_label]) == original_dataset['test']['label']))
...
for num_tokens in token_list:
    ...
    for r in np.arange(10, 201, 10):
        ...
        # approx_pred_labels[i].extend(np.argmax(approx_output.logits.detach().numpy(), axis = 1))
        #     approx_pred_probs[i].
        # output = model(**tokenizer_out)
        # eval_pred_labels.extend(np.argmax(output.logits.detach().numpy(), axis = 1))
    ...
    print('AUROC', auroc)
    print('eAUROC', eauroc)
    # if (num_tokens not in eacc_list.keys()):
    #         recovery_rate_list[str(num_tokens) + '_tokens'] = []
    #         acc_list[str(num_tokens) + '_tokens'] = []
    #         eacc_list[str(num_tokens) + '_tokens'] = []
    #         eauroc_list[str(num_tokens) + '_tokens'] = []
...
if not os.path.exists('./movies/majority_vote'):
    os.makedirs('./movies/majority_vote')
...
file = os.path.join('./movies/majority_vote/recovery_rate_stats.json')
...
file = os.path.join('./movies/majority_vote/acc_stats.json')
with open(file, "w") as outfile: 
...
file = os.path.join('./movies/majority_vote/auroc_stats.json')
with open(file, "w") as outfile:
...
if not os.path.exists('./movies/majority_vote'):
    os.makedirs('./movies/majority_vote')
file = os.path.join('./movies/majority_vote/eacc_stats.json')
with open(file, "w") as outfile: 
...
file = os.path.join('./movies/majority_vote/eauroc_stats.json')
with open(file, "w") as outfile:
